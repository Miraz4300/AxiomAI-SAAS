...
function createLogFile() {
  ...
  const logPath = path.join('/app/logs', `updateRole-${formattedDate}.log`) // Log file format: updateRole-YYYYMMDD.log
  const logStream = fs.createWriteStream(logPath, { flags: 'a' })
  logStream.write('------------------START------------------\n')
  logStream.write(`Cron job started at ${new Date().toLocaleString()}\n\n\n`)
  return { logStream, lineCount: 4 } // 4 lines have been written and start counting from 5
}
