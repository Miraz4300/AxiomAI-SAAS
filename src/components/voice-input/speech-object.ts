...
function getToken(): SpeechTokenType {
  const localState = ss.get(LOCAL_NAME)
  return { ...defaultToken(), ...localState }
}

function setToken(data: Partial<SpeechTokenType>) {
  const localState = ss.get(LOCAL_NAME)
  const newData: SpeechTokenType = { ...localState, ...data }
  ss.set(LOCAL_NAME, newData)
}

// A singleton, otherwise multiple calls would be invoked.
...
function getDefaultVoices(voices: SpeechSynthesisVoice[]): VoiceDataType[] {
...
export function useSpeechObject() {
