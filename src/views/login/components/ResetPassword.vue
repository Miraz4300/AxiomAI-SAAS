...
async function handleSendResetMail() {
  ...
  try {
    ...
    if (error.errorCode === authErrorType.NOTFOUND || error.errorCode === authErrorType.UNVERIFIED || error.errorCode === authErrorType.ABNORMAL || error.errorCode === authErrorType.PERMISSION)
